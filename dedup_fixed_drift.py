...
# -*- coding: utf-8 -*-
"""
dedup_fixed_drift.py – Fixed sequential comparison with anti-drift logic

This version uses the optimized weights (no pHash) but fixes the chain effect bug
by implementing Option 1 from driftsolns.md: compare against last KEPT frame.

Key change: Instead of comparing (i, i+1) which causes chain drift, we compare
each frame against the last frame we decided to KEEP.
...
| DRIFT_FIX | ✅ Enabled (compare vs last kept) |
...
# safety guardrails - OPTIMIZED WEIGHTS (no pHash)
MTB_HARD_FLOOR = 58.0
PDQ_HD_CEIL    = 140

# weighted-score config (weights must sum to 1.0)
WEIGHT_MTB  = 0.4
WEIGHT_SSIM = 0.1
WEIGHT_CLIP = 0.2
WEIGHT_PDQ  = 0.15
...
# weighted-score config for AERIAL photos
...
def _pair_sim(path_a: str, path_b: str) -> Tuple[float, float, int, float, float, int]:
    ...
    if mA["mtb"].shape != mB["mtb"].shape:
        logger.warning("MTB shape mismatch: %s vs %s", mA["mtb"].shape, mB["mtb"].shape)
        return 0.0, 0.0, 999, 0.0, 0.0, 0
    if mA["edges"].shape != mB["edges"].shape:
        logger.warning("Edge shape mismatch: %s vs %s", mA["edges"].shape, mB["edges"].shape)
        return 0.0, 0.0, 999, 0.0, 0.0, 0
...
def remove_near_duplicates(
    ...
    logger.info("[STEP] Multi-metric dedup with DRIFT FIX (compare vs last kept), full_scan=%s", full_scan)
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # DRIFT FIX: Compare against last KEPT frame instead of previous frame
    # ═══════════════════════════════════════════════════════════════════════════

    if full_scan:
        # Full scan mode: all-pairs comparison
        idx_pairs = [(i, j) for i in range(len(groups)-1) for j in range(i+1, len(groups))]
    else:
        # Sequential mode with drift fix: compare against last kept frame
        idx_pairs = []
        last_kept_idx = 0

        for i in range(1, len(groups)):
            # Compare current frame against last kept frame
            idx_pairs.append((last_kept_idx, i))
            # Note: We'll update last_kept_idx AFTER we process the comparison

        # We need to process comparisons differently for drift fix
        # Instead of pre-generating all pairs, we'll process on-the-fly
        logger.info("[DRIFT-FIX] Sequential mode: comparing each frame vs last KEPT")

    # Process comparisons
    if full_scan:
        # Standard full-scan processing
        ...
        # DRIFT-FIX mode: process sequentially, updating last_kept_idx
        last_kept_idx = 0

        for i in range(1, len(groups)):
            # Compare current frame (i) against last kept frame (last_kept_idx)
            mtb, edge, hd, ssim, clip, sift_matches = _pair_sim(mids[last_kept_idx], mids[i])
            if hd == 999:
                # Can't compare - keep the frame and update reference
                last_kept_idx = i
                continue

            is_aerial_i = _is_aerial(mids[i], metadata_dict)
            is_aerial_last = _is_aerial(mids[last_kept_idx], metadata_dict)
            is_aerial_pair = is_aerial_i or is_aerial_last
            ...
            for k, v in zip(("mtb", "edge", "hd", "ssim", "clip", "sift", "score"),
                            (mtb, edge, hd, ssim, clip, sift_matches, score)):
                stats[k].append(v)
            _log_pair(last_kept_idx, i, mtb, edge, hd, ssim, clip, sift_matches, score, is_aerial_pair)
            ...
            if mtb < mtb_floor and not sift_override:
                ...
                if _experiment_logger:
                    _experiment_logger.add_comparison(
                        mids[last_kept_idx], mids[i], mtb, edge, ssim, clip, hd, sift_matches, score,
                        dropped=False, drop_reason=drop_reason
                    )
                # Keep this frame and update reference
                last_kept_idx = i
                continue
            ...
            if dup:
                ...
                # DON'T update last_kept_idx - keep comparing against same reference
            else:
                # Keep frame i and update reference
                if _experiment_logger:
                    _experiment_logger.add_comparison(
                        mids[last_kept_idx], mids[i], mtb, edge, ssim, clip, hd, sift_matches, score,
                        dropped=False, drop_reason=f"SCORE < {dup_threshold}" if score < dup_threshold else ""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Near-duplicate image remover with drift fix")
    ...
    logger.info(f"Using DRIFT-FIX mode: comparing vs last KEPT frame")
