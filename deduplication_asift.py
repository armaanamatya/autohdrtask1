...
# -*- coding: utf-8 -*-
"""
deduplication_asift.py  – Image deduplication with ASIFT (Affine-SIFT) support
Near-duplicate remover that uses
    ...
    • ASIFT (Affine-SIFT for extreme viewpoint changes)

All metrics feed a **weighted composite score** with ASIFT at 10% weight.
ASIFT handles extreme viewpoint changes (up to 70-80°) better than standard SIFT.
...
class ExperimentLogger:
    ...
    def add_comparison(self, img_a: str, img_b: str, mtb: float, edge: float,
                       ...
                       asift_matches: int, score: float, dropped: bool, drop_reason: str) -> None:
...
| ASIFT_MIN_MATCHES | {ASIFT_MIN_MATCHES} |

## Results

| Image A | Image B | MTB % | Edge % | SSIM % | CLIP % | PDQ HD | SIFT | ASIFT | SCORE | Dropped? |
|---------|---------|-------|--------|--------|--------|--------|------|-------|-------|----------|
"""
        for r in self.comparison_results:
            ...
            entry += f"| {r['img_a']} | {r['img_b']} | {r['mtb']:.1f} | {r['edge']:.1f} | {r['ssim']:.1f} | {r['clip']:.1f} | {r['pdq_hd']} | {r['sift_matches']} | {r['asift_matches']} | {r['score']:.2f} | {dropped_str} |\n"
...
# Adjusted to make room for 10% ASIFT
WEIGHT_MTB  = 0.27  # Reduced from 0.30
...
AERIAL_WEIGHT_MTB  = 0.27  # Reduced from 0.30
AERIAL_WEIGHT_SSIM = 0.09  # Reduced from 0.10
AERIAL_WEIGHT_CLIP = 0.23  # Reduced from 0.25
AERIAL_WEIGHT_PDQ  = 0.23  # Reduced from 0.25
AERIAL_WEIGHT_SIFT = 0.08  # Reduced from 0.10
AERIAL_WEIGHT_ASIFT = 0.10 # NEW: ASIFT weight
AERIAL_COMPOSITE_DUP_THRESHOLD = 0.32    # 0–1 scale
AERIAL_SIFT_MIN_MATCHES = 50              # Minimum SIFT matches for aerial photos
AERIAL_ASIFT_MIN_MATCHES = 50              # Minimum ASIFT matches for aerial photos

MAX_WORKERS = 16

# ─── weight configuration helper ──────────────────────────────────────────────
def set_weights(mtb=None, ssim=None, clip=None, pdq=None, sift=None, asift=None,
                aerial_mtb=None, aerial_ssim=None, aerial_clip=None,
                aerial_pdq=None, aerial_sift=None, aerial_asift=None):
    """
    Set global weight variables for batch testing.

    Args:
        mtb, ssim, clip, pdq, sift, asift: Weights for regular photos (0.0-1.0)
        aerial_*: Weights for aerial photos (0.0-1.0)

    Note: Weights should sum to 1.0. This function does NOT validate the sum.
    """
    global WEIGHT_MTB, WEIGHT_SSIM, WEIGHT_CLIP, WEIGHT_PDQ, WEIGHT_SIFT, WEIGHT_ASIFT
    global AERIAL_WEIGHT_MTB, AERIAL_WEIGHT_SSIM, AERIAL_WEIGHT_CLIP
    global AERIAL_WEIGHT_PDQ, AERIAL_WEIGHT_SIFT, AERIAL_WEIGHT_ASIFT
    ...
    if aerial_asift is not None:
        AERIAL_WEIGHT_ASIFT = aerial_asift
...
# ─── ASIFT helpers ─────────────────────────────────────────────────────────────
def _apply_affine_transform(img: np.ndarray, tilt: float, phi: float) -> np.ndarray:
    """
    Apply affine transformation to simulate viewpoint change.
    
    Args:
        img: Input grayscale image
        tilt: Tilt parameter (1.0 = no tilt, >1.0 = more tilt)
    ...
    # Tilt transformation (affine scaling)
    # Tilt simulates camera viewpoint change
    tilt_matrix = np.array([[1.0 / tilt, 0, 0], [0, 1.0, 0]], dtype=np.float32)
    
    # Combine transformations
    combined_matrix = np.dot(rotation_matrix, np.vstack([tilt_matrix, [0, 0, 1]]))[:2, :]
    
    # Apply transformation
    transformed = cv2.warpAffine(img, combined_matrix, (w, h), 
                                 flags=cv2.INTER_LINEAR,
                                 borderMode=cv2.BORDER_REPLICATE)
    
    return transformed

def _compute_asift_matches(path_a: str, path_b: str, min_matches: int = 50) -> int:
    """
    Compute ASIFT (Affine-SIFT) feature matches between two images.
    ASIFT handles extreme viewpoint changes by simulating affine transformations.
    
    Returns the maximum number of good matches across all affine transformations.
    """
    try:
        # Check if OpenCV has AffineFeature (available in OpenCV 3.4+)
        has_affine_feature = hasattr(cv2, 'AffineFeature')
        
        if has_affine_feature:
            # Use OpenCV's built-in AffineFeature if available
            try:
                img1 = cv2.imread(path_a, cv2.IMREAD_GRAYSCALE)
                img2 = cv2.imread(path_b, cv2.IMREAD_GRAYSCALE)
                ...
                # Wrap SIFT with AffineFeature to create ASIFT
                asift = cv2.AffineFeature_create(sift)
                ...
                logger.debug(f"OpenCV AffineFeature failed, falling back to manual ASIFT: {e}")
                # Fall through to manual implementation
        else:
            logger.debug("OpenCV AffineFeature not available, using manual ASIFT simulation")
        
        # Manual ASIFT implementation: simulate affine transformations
        img1 = cv2.imread(path_a, cv2.IMREAD_GRAYSCALE)
        img2 = cv2.imread(path_b, cv2.IMREAD_GRAYSCALE)
        ...
        # ASIFT parameters: simulate different viewpoints
        # Tilt values: 1.0 (no tilt), 1.5, 2.0, 2.5, 3.0
        # Rotation angles: 0, 30, 60, 90, 120, 150 degrees
        tilt_values = [1.0, 1.5, 2.0, 2.5]
        ...
        # Try different affine transformations on both images
        for tilt1 in tilt_values:
            for phi1 in rotation_angles:
                transformed1 = _apply_affine_transform(img1, tilt1, phi1)
                kp1, des1 = sift.detectAndCompute(transformed1, None)
                
                if des1 is None or len(des1) < 2:
                    continue
                
                # Try transformations on second image
                for tilt2 in tilt_values:
                    for phi2 in rotation_angles:
                        transformed2 = _apply_affine_transform(img2, tilt2, phi2)
                        kp2, des2 = sift.detectAndCompute(transformed2, None)
                        ...
                        try:
                            ...
                            max_matches = max(max_matches, len(good_matches))
        ...
        logger.debug(f"ASIFT computation failed for {path_a} vs {path_b}: {e}")
        return 0
...
def _pair_sim(path_a: str, path_b: str) -> Tuple[float, float, int, float, float, int, int]:
    """Return (mtb %, edge %, PDQ-HD, SSIM %, CLIP %, SIFT matches, ASIFT matches)"""
    ...
    if mA["mtb"].shape != mB["mtb"].shape:
        ...
        return 0.0, 0.0, 999, 0.0, 0.0, 0, 0
    ...
    asift_matches = _compute_asift_matches(path_a, path_b, ASIFT_MIN_MATCHES)
    return mtb, edge, hd, ssim, clip, sift_matches, asift_matches
...
def remove_near_duplicates(
    ...
    logger.info("[STEP] Multi-metric dedup with ASIFT (weighted score, full_scan=%s)", full_scan)
    keep = [True] * len(groups)
    stats = {"mtb": [], "edge": [], "hd": [], "ssim": [], "clip": [], "sift": [], "asift": [], "score": []}

    def _log_pair(i: int, j: int, mtb: float, edge: float, hd: int,
                  ssim: float, clip: float, sift_matches: int, asift_matches: int, 
                  score: float, is_aerial_pair: bool):
        weight_type = "AERIAL" if is_aerial_pair else "REGULAR"
        logger.info(
            "  • %s ↔ %s : MTB=%.1f  Edge=%.1f  SSIM=%.1f  CLIP=%.1f  PDQ=%d  SIFT=%d  ASIFT=%d  SCORE=%.2f [%s]",
            Path(mids[i]).stem, Path(mids[j]).stem,
            mtb, edge, ssim, clip, hd, sift_matches, asift_matches, score, weight_type
        )
    ...
    for i, j in idx_pairs:
        ...
        mtb, edge, hd, ssim, clip, sift_matches, asift_matches = _pair_sim(mids[i], mids[j])
        if hd == 999:
        ...
        if is_aerial_pair:
            w_mtb, w_ssim, w_clip, w_pdq, w_sift, w_asift = (
                AERIAL_WEIGHT_MTB, AERIAL_WEIGHT_SSIM, AERIAL_WEIGHT_CLIP, 
                AERIAL_WEIGHT_PDQ, AERIAL_WEIGHT_SIFT, AERIAL_WEIGHT_ASIFT
            )
            ...
            asift_min = AERIAL_ASIFT_MIN_MATCHES
        else:
            w_mtb, w_ssim, w_clip, w_pdq, w_sift, w_asift = (
                WEIGHT_MTB, WEIGHT_SSIM, WEIGHT_CLIP, WEIGHT_PDQ, WEIGHT_SIFT, WEIGHT_ASIFT
            )
            dup_threshold = COMPOSITE_DUP_THRESHOLD
            mtb_floor = MTB_HARD_FLOOR
            pdq_ceil = PDQ_HD_CEIL
            sift_min = SIFT_MIN_MATCHES
            asift_min = ASIFT_MIN_MATCHES

        # Normalize SIFT and ASIFT matches to 0-1 scale (cap at 100 matches = 1.0)
        sift_score = min(sift_matches / 100.0, 1.0) if sift_matches > 0 else 0.0
        asift_score = min(asift_matches / 100.0, 1.0) if asift_matches > 0 else 0.0

        # composite score with selected weights (including ASIFT)
        score = (
            w_mtb  * (mtb  / 100.0) +
            w_ssim * (ssim / 100.0) +
            w_clip * (clip / 100.0) +
        ...
        # stats + logging
        for k, v in zip(("mtb", "edge", "hd", "ssim", "clip", "sift", "asift", "score"),
                        (mtb, edge, hd, ssim, clip, sift_matches, asift_matches, score)):
            stats[k].append(v)
        _log_pair(i, j, mtb, edge, hd, ssim, clip, sift_matches, asift_matches, score, is_aerial_pair)
        ...
        # SIFT/ASIFT override: If SIFT or ASIFT matches are high OR (SIFT moderate AND CLIP high), 
        # allow override of MTB floor AND PDQ ceiling
        sift_override = (sift_matches >= sift_min * 1.5) or ((sift_matches >= sift_min) and (clip >= 85.0))
        asift_override = (asift_matches >= asift_min * 1.5) or ((asift_matches >= asift_min) and (clip >= 85.0))
        feature_override = sift_override or asift_override
        
        if mtb < mtb_floor and not feature_override:
            ...
            if _experiment_logger:
                _experiment_logger.add_comparison(
                    mids[i], mids[j], mtb, edge, ssim, clip, hd, sift_matches, asift_matches, score,
                    dropped=False, drop_reason=drop_reason
                )
            continue
        if hd >= pdq_ceil and not feature_override:
        ...
        # Allow duplicate if: (score high AND (MTB floor passed OR feature override)) AND (PDQ ceiling passed OR feature override)
        dup = (score >= dup_threshold) and ((mtb >= mtb_floor) or feature_override) and ((hd < pdq_ceil) or feature_override)

        if feature_override:
            if sift_override and sift_matches >= sift_min * 1.5:
                trigger_metrics.append(f"SIFT_OVERRIDE(HIGH: {sift_matches}≥{sift_min * 1.5:.0f})")
            elif asift_override and asift_matches >= asift_min * 1.5:
                trigger_metrics.append(f"ASIFT_OVERRIDE(HIGH: {asift_matches}≥{asift_min * 1.5:.0f})")
            elif sift_override:
                trigger_metrics.append(f"SIFT_OVERRIDE(COMBO: SIFT={sift_matches}≥{sift_min}, CLIP={clip:.1f}≥85.0)")
            elif asift_override:
                trigger_metrics.append(f"ASIFT_OVERRIDE(COMBO: ASIFT={asift_matches}≥{asift_min}, CLIP={clip:.1f}≥85.0)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Near-duplicate image remover with ASIFT")
    ...
    logger.info(f"ASIFT enabled with {WEIGHT_ASIFT*100:.0f}% weight for extreme viewpoint handling")
